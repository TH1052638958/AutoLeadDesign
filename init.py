...
parser.add_argument('--exp-path', type=str, default='random_PRMT5_addbox=1')
...
for smile in smiles:
    score=calc_affinity(smile,dir_out=smina_path,name_protein=protein_name,cfg=args.cfg_smina)
    if(score==-500):
        continue
    dcoking_scores.append(score)
...
pd.DataFrame({'smile':smiles_filted,'dcoking_scores':dcoking_scores,
              ...
              'logp':logp_scores,'label':labels}).to_csv(os.path.join(exp_path,args.result) ,index=False)
