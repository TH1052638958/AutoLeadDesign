...
parser.add_argument('--model-engine', type=str, default='gpt-3.5-turbo')
...
for i in range(1, num_generations):
    ...
    for mol in new_molecules:
        ...
        docking_score=calc_affinity(mol,dir_out="smina",name_protein=protein_name,cfg=args.cfg_smina)
    ...
    for mol, docking_score, qed_score, radscore,mw ,logp in zip(new_molecules, filtered_docking_scores, filtered_qed_scores, filtered_rad_scores,mw_scores,logp_scores):
        if (
            ...
            labels.append('0')
    data.extend(list(zip(new_molecules, filtered_docking_scores, filtered_qed_scores, filtered_rad_scores, labels,parent)))
    # Perform further operations or analysis with the data as needed
    opath=os.path.join(exp_path,'result_{}.csv'.format(str(i)))
    df = pd.DataFrame(data, columns=['Molecule', 'Docking Score', 'QED Score', 'RADscore', 'Label', 'parent'])
